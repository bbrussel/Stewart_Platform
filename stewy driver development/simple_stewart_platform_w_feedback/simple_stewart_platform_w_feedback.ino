...
int leg1FeedbackBits;
float leg1FeedbackVoltage;
...
void setup() {
  ...
  Serial.println("Sending Platform to home position");
...
void loop() {
  ...
  if (newInputCommand == true) {
        ...
        validInput = parsePositionInputString(receivedChars, inputFloats, 6);
        if (validInput) {
          Serial.println("Moving Actuators");
  ...
  leg1FeedbackBits = analogRead(feedbackPinLeg1);

  Serial.println("Current Commanded Platform Position: ");
  platformOrientation.printOrientation();
  Serial.println(leg1FeedbackBits);
  delay(1000);
