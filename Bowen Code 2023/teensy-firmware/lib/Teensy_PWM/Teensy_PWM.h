...
#define cround(x) ((long) __builtin_round(x))
...
class Teensy_PWM
{
  public:
    ...
    {
      _dutycycle  = cround(map(dutycycle, 0, 100.0f, 0, MAX_COUNT_16BIT));
  ...
  private:
    ...
    bool setupPWM(uint32_t pin, uint32_t frequency, uint16_t dutycycle)
    {
      ...
      uint16_t newDC = cround(map(dutycycle, 0, MAX_COUNT_16BIT, 0, 1 << _resolution));
...
////////////////////////////////////////////////////////////////////////////////
  ...
  public:
    ...
    bool setPWM_Period(const uint8_t& pin, const float& period_us, const float& dutycycle)
    {
      ...
      return setPWM_Int(pin, cround(1000000.0f / period_us), _dutycycle);
    }
