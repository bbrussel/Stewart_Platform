...
class Stewart_Platform(object):   
	...
	def plot3D_line(s, ax, vec_arr_origin, vec_arr_dest, color_):
		...
		for i in range(6):
			...
			ax.text(vec_arr_origin[0, i],vec_arr_origin[1, i],vec_arr_origin[2, i],  '%s' % (str(i+1)), size=14, zorder=1,  color='#586e75')

			ax.text(vec_arr_dest[0, i],vec_arr_dest[1, i],vec_arr_dest[2, i],  '%s' % (str(i+1)), size=14, zorder=1,  color='#586e75')
